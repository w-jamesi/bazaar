...
# Direct Vercel deployment using API
...
# Create a tar file for deployment
echo "📦 Creating deployment package..."
cd "$TEMP_DIR"
tar -czf ../deployment.tar.gz . > /dev/null
cd - > /dev/null
...
DEPLOY_RESPONSE=$(curl -s -X POST "https://api.vercel.com/v13/deployments" \
  -H "Authorization: Bearer $VERCEL_TOKEN" \
  -F "files=@$TEMP_DIR/../deployment.tar.gz" \
  -F "name=$PROJECT_NAME" \
...
# Clean up
rm -rf "$TEMP_DIR"
rm -f "$TEMP_DIR/../deployment.tar.gz"
